...
from ugeeapp.models import StopEvent,Production,MaterialLoss,SKU,ProcessParams,StopCode,ReasonOne,ReasonTwo,ReasonTri,ReasonFour,Equipment,User,SAFETY_BOS,QA_BOS,OGC_BOS
from ugeeapp.forms import LoginForm,PSGProductionEntryForm,WHSE_RPMBOSForm,ViewPSGResultForm,PSGProductionForm,SKUEntryForm,EquipmentEntryForm,EditUserForm,ChangePasswordForm,StopsForm,AddUserForm,forgotPasswordForm,OGCBOSForm,QABOSForm,SAFETYBOSForm,ViewBOSForm,LeadershipBOSForm,MSG_OGCBOSForm,MSG_QABOSForm,MSG_SAFETYBOSForm,WHSE_QABOSForm,WHSE_SAFETYBOSForm
...
def userview():
	...
	response=''
	userinfo=db.session.query(User).order_by(User.userid.desc()).all()
...
def equipment():
	...
	#form.sku.choices=[(g.skuid,g.description) for g in SKU.query.order_by(SKU.weight.desc())]
	response=''
...
def my_e_learning():
	...
	if request.args.get('action') == 'ADD-TRAINING':
		...
		if request.method == 'POST':
			...
			records = data['data']
			title = "Training records for: {}".format(data['title'])

		return render_template(template,records=records, title=title,courses=courses)
...
def uvaa():
	...
	if request.args.get("shift")=="current":
		...
		myevent=db.session.query(StopEvent).filter(StopEvent.machine==mach).order_by(StopEvent.start_time.desc()).all()
		count=0
